...
async function downloadSharePointFile(id: string, fileName: string) {
  const { driveId, accessToken } = await getSharePointDrive();
  const headers = { Authorization: `Bearer ${accessToken}` };
  const metadataResponse = await fetch(`https://graph.microsoft.com/v1.0/drives/${encodeURIComponent(driveId)}/items/${encodeURIComponent(id)}?$select=id,name,@microsoft.graph.downloadUrl`, { headers });
  if (!metadataResponse.ok) throw new Error(`No se pudo consultar el archivo en SharePoint (${metadataResponse.status})`);
  const metadata = await metadataResponse.json() as { name?: string; [key: string]: unknown };
  const downloadUrl = metadata["@microsoft.graph.downloadUrl"];
  if (typeof downloadUrl === "string" && downloadUrl) {
    const anchor = document.createElement("a"); anchor.href = downloadUrl; anchor.download = fileName || metadata.name || "archivo"; anchor.target = "_blank"; anchor.rel = "noopener"; document.body.appendChild(anchor); anchor.click(); anchor.remove(); return;
  }
  const response = await fetch(`https://graph.microsoft.com/v1.0/drives/${encodeURIComponent(driveId)}/items/${encodeURIComponent(id)}/content?download=true`, { headers });
  if (!response.ok) throw new Error(`No se pudo descargar el archivo (${response.status})`);
  const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = fileName; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 60_000);
}
...
export default function Home() {
  ...
  const syncMicrosoftCalendar = async (account: AccountInfo) => {
    ...
    const token = await microsoftApp.acquireTokenSilent({ account, scopes: ["Calendars.Read"] });
      ...
      const imported: Meeting[] = payload.value.map((event) => ({ id: event.id, title: event.subject || "Reunión de Outlook", organizerName: event.organizer?.emailAddress?.name || "Mi calendario Microsoft 365", scheduledAt: event.start?.dateTime ? new Date(`${event.start.dateTime}Z`).toISOString() : new Date().toISOString(), durationMinutes: event.start?.dateTime && event.end?.dateTime ? Math.max(15, Math.round((new Date(`${event.end.dateTime}Z`).getTime() - new Date(`${event.start.dateTime}Z`).getTime()) / 60000)) : 60, attendeesCount: event.attendees?.length || 0, attendees: (event.attendees || []).map((attendee) => { const item = attendee as { emailAddress?: { name?: string; address?: string }; role?: string; jobTitle?: string; cargo?: string; title?: string }; const name = item.emailAddress?.name || item.emailAddress?.address || "Asistente"; const normalized = name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); const knownRole = normalized.includes("juan david gil rodriguez") || item.emailAddress?.address?.toLowerCase() === "jgilr@abcrepecev.com" ? "Desarrollador" : item.jobTitle || item.cargo || item.title || item.role || "Cargo no informado"; return { name, email: item.emailAddress?.address || "", role: knownRole }; }), recordingEnabled: false, processingEnabled: true, status: event.isOnlineMeeting ? "Teams" : "Calendario", source: "Microsoft Graph" }));
      setMeetings(imported);
      setConnected(true);
...
function Documents({ data, recordings, sharePointItems, sharePointLoading, preview, actaStatus, onLoadSharePoint, onEditPreview, onSendSharePoint, onGenerateFromRecording, onTest, onDelete, attendees, onUpload }: { data?: Array<{ id: number; kind: string; format: string; fileName: string; storageUrl: string | null; meetingTitle?: string }>; recordings?: RecordingItem[]; sharePointItems: Array<{ id: string; note: string; modified: string }>; sharePointLoading: boolean; preview: string; actaStatus: { type: string; text: string }; onLoadSharePoint: () => void; onEditPreview: (value: string) => void; onSendSharePoint: () => void; onGenerateFromRecording: (id: number, url: string) => void; onTest: () => void; onDelete: (id: number) => void; attendees: Attendee[]; onUpload: (event: React.ChangeEvent<HTMLInputElement>) => void }) {
  const [emailOpen, setEmailOpen] = useState(false);
  const [selectedEmails, setSelectedEmails] = useState<string[]>([]);
  const [selectedMinutes, setSelectedMinutes] = useState<number[]>([]);
  const [activeMinuteId, setActiveMinuteId] = useState<number | null>(null);
