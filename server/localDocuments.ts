...
function actaLines(title: string, output: MeetingOutput, attendees: Array<{ name: string; email: string; role?: string }> = []) {
...
export async function makeLocalDocuments(input: { meetingTitle: string; output: MeetingOutput; attendees?: Array<{ name: string; email: string; role?: string }> }) {
  const base = `${cleanFileName(input.meetingTitle)}-${new Date().toISOString().slice(0, 10)}`;
  const lines = actaLines(input.meetingTitle, input.output, input.attendees);
